...
export function processRailwayPNR(pnrData) {
  ...
  const { pnr, train, passengers, classBooked } = pnrData;
  ...
  function getStatusLabel(current) {
    if (typeof current !== "string") return "WAITING";
  ...
  const processedPassengers = passengers.map((p) => {
    const statusLabel = getStatusLabel(p.current);
